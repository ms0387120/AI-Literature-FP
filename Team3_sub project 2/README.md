...
---
# Method
* 根據 Project 1 對文章抽取的 **人物**、**地點**、**對話**、**事件** 進行劇本生成

    - 抽取方法與 Project 1 相同
    - 為了追求提取更高的元素準確度，有待在 sub-project 3 時從大量文本中訓練

* Scene 採用方式 **「電影式」切景** 

    - 依照「時」、「空」分景，目前不增刪文本劇情

        - **時間：「清晨」、「白天」、「黃昏」、「晚上」（根據劇作家講者介紹）**
        - **空間：根據元素抽取的 Location**
    
    - 每句話都會紀錄其時空，只要跟前期狀態不同，**尤其是對於地點特別敏感**，立即切換場景
    - 切換 Scene應注意 **「地點在文本中所屬位置會影響分景正確度」**
    - **偵測對話框內的地點 v.s. 偵測對話框外的地點**

        - **撰寫出對話地點判斷器**，來取決是否採用對話框內還是外的地點
        - 以三隻小豬為例，以**對話內**的地點為目標。因為在故事裡，三隻小豬蓋的建築物大多以在對話內出現次數最多（明顯多於對話框外），因此以對話內容當作依據來切換場景，會比較準確
        - 以小紅帽、女水妖為例，以**對話外**的地點為偵測目標。因為故事發生地點多在對話外就會顯示，不會在對話內容說明。就算有，也擔心會有偏誤。比方說：小紅帽在森林（對話外）時，一直說要去奶奶家（對話內）
...
> • 準確率計算方式：直接由生成的劇本中，正確的句子占所有句子的百分比比率
...
* Error 發生在
    ...
    - 講話的人寫錯
        ...
        - 小紅帽的媽媽：我會小心的
            - 更正是小紅帽對媽媽說
            - 錯誤原因： StanfordNLP 主詞/受詞 判斷錯誤，無從修改，因為牽一髮而動全身
 
        - 三隻小豬的野狼：萬歲
            - 更正是三隻小豬說
            - 錯誤原因：不知道「三隻小豬」這個群體，只知道三個個體（代名詞問題）

* Scene 切割對地點特別敏感，時間其次

    - 因為在故事中時間可能不像地點特別說明出來
...
* 對話句子間的敘述有留意人物動作
* 對話說話人準確度比上次略有提高 （90% -> 97%）

---
# Conclusion
* 生成出最原始的劇本（未經修改）
* 對話生成取決「人物」是否抓的正確、主詞關係
* 切換場景取決「地點」、「時間」

## Future Work
* 提升故事中元素準確度，尤其是「人物」、「地點」
...
* 以上是這次作業的簡單報告，而這次作業發生的缺點，希望能在 sub-project 3 進行改進
